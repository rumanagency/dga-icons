...
const JS_SRC = join(ROOT, 'packages', 'js', 'src');

async function main() {
  console.log('🟨  Generating @dga-icons/js source files...\n');
  ...
  console.log(`\n✅ @dga-icons/js generation complete in ${elapsed}s`);
}
...
async function generateForStyle(style, isDefault) {
  ...
  const indexDir = isDefault ? JS_SRC : join(JS_SRC, style);
...
function generateComponentSource(
  ...
  const createJsIconPath = isDefault
    ? '../createJsIcon'
    : '../../createJsIcon';

  return `import { createJsIcon } from '${createJsIconPath}';
import { ${componentName} as iconData } from '${coreImportPath}';

const ${componentName} = createJsIcon('${iconName}', iconData);
...
function generateBarrelIndex(iconNames, isDefault) {
  ...
  if (isDefault) {
    ...
    lines.push("export { createJsIcon } from './createJsIcon';");
