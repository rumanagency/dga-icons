import type { IconNode } from '../../types';

const Hugeicons: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M6.025 8.5h11.95c-.274-2.748-2.782-5-5.975-5s-5.701 2.252-5.975 5m13.958 0H23v2h-1.617l-1.062 8.496a4 4 0 0 1-3.97 3.504H7.649a4 4 0 0 1-3.969-3.504L2.617 10.5H1v-2h3.017c.276-3.968 3.81-7 7.983-7 4.174 0 7.707 3.032 7.983 7",
        "clipRule": "evenodd"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M5.77 8h11.96c-.28-2.75-2.79-5-5.98-5s-5.7 2.25-5.98 5m13.96 0h3.02v2h-1.62l-1.06 8.5c-.25 2-1.95 3.5-3.97 3.5H7.4c-2.02 0-3.72-1.5-3.97-3.5L2.37 10H.75V8h3.02c.27-3.97 3.81-7 7.98-7s7.71 3.03 7.98 7",
        "clipRule": "evenodd"
      }
    ]
  ];

export default Hugeicons;
