import type { IconNode } from '../../types';

const CloudAngledRainZap: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M5.859 7.106c-2.604.503-4.609 2.7-4.609 5.39 0 1.815.92 3.414 2.315 4.408a.75.75 0 1 0 .87-1.22c-1.033-.738-1.685-1.897-1.685-3.189 0-2.045 1.652-3.781 3.838-3.979q.203-.018.412-.018c.967 0 1.854.303 2.565.81a.75.75 0 1 0 .871-1.221 5.9 5.9 0 0 0-3.01-1.074C7.983 5.15 9.797 3.75 12 3.75c2.66 0 4.75 2.039 4.75 4.472q0 .206-.02.404a4.3 4.3 0 0 1-1.053 2.428.75.75 0 0 0 1.132.984 5.85 5.85 0 0 0 1.3-2.545c1.807.275 3.141 1.756 3.141 3.477 0 1.508-1.017 2.825-2.489 3.319a.75.75 0 1 0 .478 1.422c2.023-.68 3.511-2.528 3.511-4.74 0-2.563-1.985-4.628-4.504-4.973C18.122 4.77 15.338 2.25 12 2.25c-3.025 0-5.59 2.066-6.141 4.856",
        "clipRule": "evenodd"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M9.492 12.874a.75.75 0 0 1 .212 1.04L8.16 16.25h1.139c.181 0 .392 0 .565.026.188.027.569.119.777.511.207.39.072.756-.008.926a4 4 0 0 1-.291.487l-.02.03-1.774 2.684a.75.75 0 1 1-1.252-.828l1.545-2.336H7.702c-.181 0-.392 0-.566-.025-.187-.028-.568-.12-.776-.512-.207-.39-.072-.756.008-.926.074-.158.19-.334.291-.487l.02-.03 1.774-2.684a.75.75 0 0 1 1.04-.212m6.844 4.955a.75.75 0 0 1 .335 1.007l-1 2a.75.75 0 0 1-1.342-.671l1-2a.75.75 0 0 1 1.007-.336m-2.665 1.007a.75.75 0 0 0-1.342-.671l-1 2a.75.75 0 0 0 1.342.67z",
        "clipRule": "evenodd",
        "opacity": ".4"
      }
    ]
  ];

export default CloudAngledRainZap;
