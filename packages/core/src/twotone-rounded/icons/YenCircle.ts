import type { IconNode } from '../../types';

const YenCircle: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M12 2.75a9.25 9.25 0 1 0 0 18.5 9.25 9.25 0 0 0 0-18.5M1.25 12C1.25 6.063 6.063 1.25 12 1.25S22.75 6.063 22.75 12 17.937 22.75 12 22.75 1.25 17.937 1.25 12",
        "clipRule": "evenodd"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M8.84 6.767a.75.75 0 0 1 .893.573c.162.741.854 2.375 2.267 3.293 1.413-.918 2.106-2.552 2.267-3.293a.75.75 0 1 1 1.466.32c-.211.968-1.073 3.072-2.983 4.274v.816h1.75a.75.75 0 0 1 0 1.5h-1.75v2.25a.75.75 0 0 1-1.5 0v-2.25H9.5a.75.75 0 0 1 0-1.5h1.75v-.816C9.34 10.732 8.48 8.628 8.267 7.66a.75.75 0 0 1 .573-.893",
        "clipRule": "evenodd",
        "opacity": ".4"
      }
    ]
  ];

export default YenCircle;
