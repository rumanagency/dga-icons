import type { IconNode } from '../../types';

const LayerMask02: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M12 2.75a9.25 9.25 0 1 0 0 18.5 9.25 9.25 0 0 0 0-18.5M1.25 12C1.25 6.063 6.063 1.25 12 1.25S22.75 6.063 22.75 12 17.937 22.75 12 22.75 1.25 17.937 1.25 12",
        "clipRule": "evenodd"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M14.25 12.003a9.25 9.25 0 0 0-6.012-8.668l.525-1.405c4.08 1.524 6.988 5.458 6.988 10.073 0 4.614-2.907 8.548-6.988 10.073l-.525-1.405a9.25 9.25 0 0 0 6.013-8.668",
        "clipRule": "evenodd",
        "opacity": ".4"
      }
    ]
  ];

export default LayerMask02;
