import type { IconNode } from '../../types';

const Twitter: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M11.258 8.238c-.113-2.602 2-4.988 4.742-4.988 1.075 0 2.067.358 2.863.96l3.027-.452a.75.75 0 0 1 .778 1.083l-1.933 3.792C20.405 15.381 14.829 20.75 8 20.75c-2.295 0-4.478-.503-6.375-1.6a.75.75 0 0 1 .3-1.396c.702-.071 1.764-.273 2.63-.589.438-.16.779-.33.997-.497l.057-.046c-3.035-3.338-4.101-8.45-2.841-12.729a.75.75 0 0 1 1.351-.191c1.61 2.521 4.247 4.393 7.14 4.536",
        "opacity": ".4"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M11.259 8.238c-.114-2.602 2-4.988 4.742-4.988 1.074 0 2.067.358 2.863.96l3.026-.452a.75.75 0 0 1 .779 1.083l-1.934 3.792C16.501 18.343 6.001 19 1.268 18.343a.75.75 0 0 1 .658-.59c.702-.07 1.764-.272 2.63-.588.438-.16.779-.33.996-.497l.057-.046c-3.034-3.338-4.1-8.45-2.84-12.729a.75.75 0 0 1 1.35-.191c1.61 2.521 4.248 4.393 7.14 4.536"
      }
    ]
  ];

export default Twitter;
