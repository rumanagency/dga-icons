import type { IconNode } from '../../types';

const TvSmart: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M14.056 2.25H9.944c-1.838 0-3.294 0-4.433.153-1.172.158-2.121.49-2.87 1.238-.748.749-1.08 1.698-1.238 2.87-.153 1.14-.153 2.595-.153 4.433v.112c0 1.838 0 3.294.153 4.433.158 1.172.49 2.121 1.238 2.87.749.748 1.698 1.08 2.87 1.238 1.14.153 2.595.153 4.433.153h4.112c1.838 0 3.294 0 4.433-.153 1.172-.158 2.121-.49 2.87-1.238.748-.749 1.08-1.698 1.238-2.87.153-1.14.153-2.595.153-4.433v-.112c0-1.838 0-3.294-.153-4.433-.158-1.172-.49-2.121-1.238-2.87-.749-.748-1.698-1.08-2.87-1.238-1.14-.153-2.595-.153-4.433-.153",
        "opacity": ".4"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M11.25 15.5a7.25 7.25 0 0 1 7.25-7.25.75.75 0 0 0 0-1.5 8.75 8.75 0 0 0-8.75 8.75.75.75 0 0 0 1.5 0m3.2 0a4.05 4.05 0 0 1 4.05-4.05.75.75 0 0 0 0-1.5 5.55 5.55 0 0 0-5.55 5.55.75.75 0 0 0 1.5 0m3.2 0c0-.47.38-.85.85-.85a.75.75 0 0 0 0-1.5 2.35 2.35 0 0 0-2.35 2.35.75.75 0 0 0 1.5 0m.839 4.095q-.571.076-1.253.108l.87 1.739a1 1 0 1 0 1.789-.894l-.555-1.11q-.404.098-.851.157m-12.978 0a8 8 0 0 1-.85-.157l-.555 1.11a1 1 0 1 0 1.789.894l.869-1.739a15 15 0 0 1-1.253-.108",
        "clipRule": "evenodd"
      }
    ]
  ];

export default TvSmart;
