import type { IconNode } from '../../types';

const Spoon: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M17.357 1.373c1.504-.287 3.17-.118 4.278.991 1.109 1.109 1.278 2.774.991 4.279-.29 1.525-1.076 3.08-2.155 4.159-1.197 1.197-2.556 1.603-3.9 1.342-1.039-.203-1.895-.108-2.362.39l-8.902 9.475a2.347 2.347 0 1 1-3.317-3.317l9.476-8.901c.498-.468.592-1.324.39-2.363-.262-1.344.145-2.702 1.341-3.9 1.08-1.079 2.635-1.865 4.16-2.155",
        "opacity": ".4"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M17.36 1.373c1.504-.287 3.17-.118 4.278.991 1.109 1.109 1.278 2.774.991 4.279-.29 1.525-1.076 3.08-2.156 4.159-1.197 1.197-2.555 1.603-3.899 1.342-1.039-.203-1.895-.108-2.363.39L11.47 9.79c.498-.468.592-1.324.39-2.363-.262-1.344.144-2.702 1.341-3.9 1.08-1.079 2.635-1.865 4.16-2.155"
      }
    ]
  ];

export default Spoon;
