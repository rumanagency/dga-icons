import type { IconNode } from '../../types';

const Time01: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M12.057 1.75c2.19 0 3.912 0 5.255.18 1.377.186 2.469.573 3.327 1.431s1.245 1.95 1.43 3.327c.181 1.344.181 3.064.181 5.255v.114c0 2.191 0 3.912-.18 5.255-.186 1.377-.573 2.47-1.43 3.327-.86.858-1.951 1.246-3.328 1.43-1.343.181-3.064.181-5.255.181h-.114c-2.19 0-3.912 0-5.255-.18-1.377-.185-2.469-.573-3.327-1.43-.858-.859-1.245-1.951-1.43-3.328-.181-1.343-.181-3.064-.181-5.255v-.114c0-2.19 0-3.911.18-5.255.186-1.376.573-2.469 1.43-3.327.86-.858 1.951-1.245 3.328-1.43 1.343-.18 3.064-.18 5.255-.18z",
        "opacity": ".4"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M12 6a1 1 0 0 1 1 1v2.708a2.5 2.5 0 0 1 1.328 3.206l1.38 1.38a1 1 0 0 1-1.415 1.413l-1.38-1.379A2.5 2.5 0 0 1 11 9.708V7a1 1 0 0 1 1.001-1"
      }
    ]
  ];

export default Time01;
