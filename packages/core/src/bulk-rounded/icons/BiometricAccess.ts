import type { IconNode } from '../../types';

const BiometricAccess: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M7.525 8.35A96 96 0 0 1 12 8.25c1.596 0 3.046.034 4.475.1 2.058.095 3.729 1.688 4 3.705.149 1.1.275 2.257.275 3.445s-.126 2.344-.274 3.445c-.272 2.017-1.943 3.61-4.001 3.705-1.429.066-2.88.1-4.475.1-1.596 0-3.046-.034-4.475-.1-2.058-.095-3.729-1.688-4-3.705-.149-1.1-.275-2.257-.275-3.445s.126-2.344.275-3.445c.271-2.017 1.942-3.61 4-3.705",
        "opacity": ".4"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M12 2.75A3.75 3.75 0 0 0 8.25 6.5v1.82q-.364.014-.725.03-.398.019-.775.11V6.5a5.25 5.25 0 1 1 10.5 0v1.96q-.376-.091-.775-.11-.361-.016-.725-.03V6.5A3.75 3.75 0 0 0 12 2.75m0 10A3.25 3.25 0 0 0 8.75 16v1.5a.75.75 0 0 1-1.5 0V16a4.75 4.75 0 1 1 9.5 0v1.5a.75.75 0 0 1-1.5 0V16A3.25 3.25 0 0 0 12 12.75m0 2.5a.75.75 0 0 0-.75.75v3a.75.75 0 0 1-1.5 0v-3a2.25 2.25 0 0 1 4.5 0 .75.75 0 0 1-1.5 0 .75.75 0 0 0-.75-.75m1.5 2a.75.75 0 0 1 .75.75v1a.75.75 0 0 1-1.5 0v-1a.75.75 0 0 1 .75-.75",
        "clipRule": "evenodd"
      }
    ]
  ];

export default BiometricAccess;
