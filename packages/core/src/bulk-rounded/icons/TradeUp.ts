import type { IconNode } from '../../types';

const TradeUp: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M20.707 7.293a1 1 0 0 1 0 1.414l-5.038 5.038c-.409.409-.776.776-1.111 1.04-.365.286-.792.535-1.334.584q-.225.02-.448 0c-.542-.05-.97-.298-1.334-.584-.335-.264-.703-.631-1.11-1.04-.46-.458-.78-.777-1.01-.958a.48.48 0 0 0-.643 0c-.23.181-.513.461-.972.92l-3 3a1 1 0 0 1-1.414-1.414l3.038-3.038c.409-.409.776-.776 1.111-1.04.365-.286.792-.535 1.334-.584q.225-.02.448 0c.542.05.97.298 1.334.584.335.264.703.631 1.11 1.04.46.458.78.777 1.01.958.112.09.397.215.643 0 .23-.181.513-.461.972-.92l5-5a1 1 0 0 1 1.414 0",
        "opacity": ".4"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M14 8a1 1 0 0 1 1-1h5a1 1 0 0 1 1 1v5a1 1 0 1 1-2 0V9h-4a1 1 0 0 1-1-1",
        "clipRule": "evenodd"
      }
    ]
  ];

export default TradeUp;
