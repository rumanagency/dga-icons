import type { IconNode } from '../../types';

const Envato: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M5.09 9.548c-.048 1.43.13 3.013.735 4.35.304.672.456 1.008.727.961.271-.046.3-.467.355-1.309.366-5.523 5.16-11.418 10.764-12.228.565-.082.848-.123 1.18.04.333.165.477.432.765.966 4.818 8.937 1.2 20.422-8.07 20.422C4.252 22.75-.1 14.357 3.59 8.865c.588-.875.882-1.313 1.22-1.21s.318.7.28 1.893",
        "opacity": ".4"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M5.09 9.548c-.048 1.43.13 3.013.735 4.35.304.672.456 1.008.727.961.271-.046.3-.467.355-1.309.366-5.523 5.16-11.418 10.764-12.228.565-.082.848-.123 1.18.04C18 7 15 17.5 8 22.034 2.97 19.89.548 13.391 3.59 8.865c.588-.875.882-1.313 1.22-1.21s.318.7.28 1.893"
      }
    ]
  ];

export default Envato;
