import type { IconNode } from '../../types';

const Rotate01: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M12 3a9 9 0 1 0 9 9 1 1 0 1 1 2 0c0 6.075-4.925 11-11 11S1 18.075 1 12 5.925 1 12 1c2.168 0 4.19.628 5.894 1.711l1.328-1.328A1 1 0 0 1 21.01 2v3.132c0 1.222-1.518 1.779-2.309.86A8.98 8.98 0 0 0 12 3",
        "opacity": ".4"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M12 3a9 9 0 0 0-6.5 15.225L4 19.55C2.14 17.58 1 14.923 1 12 1 5.925 5.925 1 12 1c2.168 0 4.19.628 5.894 1.711l1.328-1.328A1 1 0 0 1 21.01 2v3.132c0 1.222-1.518 1.779-2.309.86A8.98 8.98 0 0 0 12 3"
      }
    ]
  ];

export default Rotate01;
