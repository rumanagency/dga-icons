import type { IconNode } from '../../types';

const SmartphoneLostWifi: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M7.729 5.266c.13-.004.25.06.332.161a2.48 2.48 0 0 0 1.319.845c.23.059.345.088.387.147s.035.177.02.414a2.5 2.5 0 0 0 .37 1.48 2.5 2.5 0 0 0 1.872 1.171c.24.025.36.037.41.085l.019.02c.042.055.042.175.042.416a2.5 2.5 0 0 0 2.5 2.501h.006q.107 0 .212-.009c.269-.022.403-.034.468.026.064.06.064.177.064.414v3.525c0 1.274 0 2.306-.107 3.118-.112.845-.352 1.564-.917 2.137-.566.575-1.28.82-2.116.935-.802.11-1.82.11-3.074.11h-.11c-1.255 0-2.273 0-3.075-.11-.837-.115-1.55-.36-2.116-.935-.716-.727-.913-1.694-.983-2.863l-.002-.025.08-10.485.009-.054c.12-.783.364-1.453.896-1.993.566-.575 1.28-.821 2.116-.935.404-.055.862-.083 1.378-.096M8.5 19.009a1 1 0 0 1 1-1h.009a1 1 0 0 1 0 2H9.5a1 1 0 0 1-1-1",
        "clipRule": "evenodd",
        "opacity": ".4"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M13.293 2.293a1 1 0 0 0-.152 1.219 10 10 0 0 0-.774.572 1 1 0 0 0 1.265 1.549q.45-.366.931-.656L16.1 6.514a6.4 6.4 0 0 0-1.346.637 1 1 0 0 0 1.058 1.698 4.15 4.15 0 0 1 1.981-.642l2.5 2.5a1 1 0 0 0 1.415-1.414l-7-7a1 1 0 0 0-1.414 0M18 10a1 1 0 1 0 0 2 1.01 1.01 0 0 0 1.006-1c0-.55-.459-1-1.006-1m.007-8c2.098 0 4.03.775 5.624 2.071a1 1 0 0 1-1.262 1.552C21.09 4.583 19.593 4 18.007 4q-.222 0-.44.015a1 1 0 1 1-.135-1.995q.285-.02.575-.02",
        "clipRule": "evenodd"
      }
    ]
  ];

export default SmartphoneLostWifi;
