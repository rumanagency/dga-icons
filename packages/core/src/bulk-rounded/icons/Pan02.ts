import type { IconNode } from '../../types';

const Pan02: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M8.75 13.556c0-.557.387-1.306 1.194-1.306h11.112c.807 0 1.194.75 1.194 1.306 0 2.704-1.78 5.194-4.306 5.194h-4.888c-2.526 0-4.306-2.49-4.306-5.194"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M7.027 11.592c-.73.31-1.607.166-2.202-.43L2.338 8.676a2.006 2.006 0 1 1 2.837-2.837l2.487 2.487c.597.597.74 1.476.427 2.208l2.684 2.684a.75.75 0 1 1-1.06 1.06z",
        "clipRule": "evenodd",
        "opacity": ".4"
      }
    ]
  ];

export default Pan02;
