import type { IconNode } from '../../types';

const Refresh: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M12 3.205A8.795 8.795 0 1 0 20.796 12a.977.977 0 1 1 1.954 0c0 5.937-4.813 10.75-10.75 10.75S1.25 17.937 1.25 12 6.063 1.25 12 1.25c2.125 0 4.107.617 5.775 1.682L19 1.707a.976.976 0 0 1 1.805.52v3.061c0 1.194-1.484 1.738-2.257.84A8.77 8.77 0 0 0 12 3.205",
        "opacity": ".4"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M12 3.205a8.795 8.795 0 0 0-7.507 13.38l-1.517 1.26A10.7 10.7 0 0 1 1.25 12C1.25 6.063 6.063 1.25 12 1.25c2.125 0 4.107.617 5.775 1.682L19 1.707a.976.976 0 0 1 1.805.52v3.061c0 1.194-1.484 1.738-2.257.84A8.77 8.77 0 0 0 12 3.205"
      }
    ]
  ];

export default Refresh;
