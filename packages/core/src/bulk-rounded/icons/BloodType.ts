import type { IconNode } from '../../types';

const BloodType: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M9.078 2.428a4.213 4.213 0 0 1 5.844 0c1.28 1.216 2.849 2.892 4.102 4.814 1.247 1.913 2.226 4.138 2.226 6.436 0 4.468-3.507 9.072-9.25 9.072s-9.25-4.604-9.25-9.072c0-2.298.979-4.523 2.226-6.436C6.23 5.32 7.798 3.644 9.078 2.428",
        "opacity": ".4"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M12 8.25a.75.75 0 0 1 .671.415l2.136 4.272.014.028.85 1.7a.75.75 0 1 1-1.342.67l-.65-1.3h-3.358l-.65 1.3a.75.75 0 1 1-1.342-.67l.85-1.7.014-.028 2.136-4.272A.75.75 0 0 1 12 8.25m-.93 4.286h1.86L12 10.677z",
        "clipRule": "evenodd"
      }
    ]
  ];

export default BloodType;
