import type { IconNode } from '../../types';

const CenterFocus: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M3.5 4.247c-1 1.363-1 3.405-1 7.489s0 6.126 1 7.49q.24.328.527.602c1.192 1.144 2.977 1.144 6.548 1.144h2.85c3.57 0 5.356 0 6.548-1.144q.286-.274.527-.603c1-1.363 1-3.405 1-7.489s0-6.126-1-7.49a4.6 4.6 0 0 0-.527-.602C18.78 2.5 16.996 2.5 13.425 2.5h-2.85c-3.57 0-5.356 0-6.548 1.144q-.286.275-.527.603M12 16a4 4 0 1 0 0-8 4 4 0 0 0 0 8",
        "clipRule": "evenodd",
        "opacity": ".4"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M9.151 3.25a.75.75 0 0 0-.034-1.5c-2.638.06-4.498.296-5.8 1.594-1.146 1.142-1.46 2.705-1.566 4.819a.75.75 0 0 0 1.498.074c.103-2.071.41-3.117 1.127-3.83.826-.824 2.081-1.096 4.775-1.157m5.732-1.5a.75.75 0 1 0-.034 1.5c2.694.061 3.949.333 4.775 1.156.716.714 1.024 1.76 1.127 3.831a.75.75 0 0 0 1.498-.074c-.105-2.114-.42-3.677-1.566-4.82-1.302-1.297-3.162-1.533-5.8-1.593M3.249 15.763a.75.75 0 1 0-1.498.074c.105 2.114.42 3.677 1.566 4.82 1.302 1.297 3.162 1.533 5.8 1.593a.75.75 0 0 0 .034-1.5c-2.694-.061-3.949-.333-4.775-1.156-.716-.714-1.024-1.76-1.127-3.831m19 .074a.75.75 0 1 0-1.498-.074c-.103 2.071-.41 3.117-1.127 3.83-.826.824-2.081 1.096-4.775 1.157a.75.75 0 0 0 .034 1.5c2.638-.06 4.498-.296 5.8-1.594 1.146-1.142 1.461-2.705 1.566-4.819M8.75 12a3.25 3.25 0 1 1 6.5 0 3.25 3.25 0 0 1-6.5 0M12 7.25a4.75 4.75 0 1 0 0 9.5 4.75 4.75 0 0 0 0-9.5",
        "clipRule": "evenodd"
      }
    ]
  ];

export default CenterFocus;
