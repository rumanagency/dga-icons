import type { IconNode } from '../../types';

const Queue02: IconNode = [
    [
      "path",
      {
        "fill": "currentColor",
        "d": "M17.75 18h-12c-.943 0-1.414 0-1.707-.293S3.75 16.943 3.75 16s0-1.414.293-1.707S4.807 14 5.75 14h12c.943 0 1.414 0 1.707.293s.293.764.293 1.707 0 1.414-.293 1.707-.764.293-1.707.293",
        "opacity": ".4"
      }
    ],
    [
      "path",
      {
        "fill": "currentColor",
        "fillRule": "evenodd",
        "d": "M3.75 6.75h16a.75.75 0 0 0 0-1.5h-16a.75.75 0 0 0 0 1.5m16 4h-16a.75.75 0 0 1 0-1.5h16a.75.75 0 0 1 0 1.5m-2 8h-12c-1.075 0-1.725 0-2.237-.513C3 17.724 3 17.075 3 16s0-1.725.513-2.237c.513-.513 1.162-.513 2.237-.513h12c1.075 0 1.725 0 2.237.513.513.513.513 1.162.513 2.237s0 1.725-.513 2.237c-.513.513-1.162.513-2.237.513m-12-4c-.501 0-1.069 0-1.177.074-.073.107-.073.675-.073 1.176s0 1.069.074 1.177c.107.073.675.073 1.176.073h12c.501 0 1.069 0 1.177-.074C19 17.069 19 16.501 19 16s0-1.069-.074-1.177c-.107-.073-.675-.073-1.176-.073z",
        "clipRule": "evenodd"
      }
    ]
  ];

export default Queue02;
